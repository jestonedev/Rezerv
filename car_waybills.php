...
if (Auth::hasPrivilege(AUTH_MANAGE_TRANSPORT)) {
    ...
    <div class="cars__page-header page-header text-center">
        <h1><small>Путевые листы автомобиля <?=$carInfo["number"]?></small></h1>
    </div>
    <?php
        $page = 1;
    ...
    <div class="col-md-10 col-md-offset-1">
        <table class="table table-striped table-hover car__table">
            ...
            foreach ($waybills as $waybill) {
                ...
                <td>
                <div class="btn-group">
                <a title="Детальная информация" href="waybill_details.php?id_waybill=<?=$waybill["id_waybill"]?>" class="btn btn-default">
        ...
        <ul class="pagination">
            ...
            <?php
                for($i = 1; $i <= $waybillsPageCount; $i++)
                {
                    ...
                    ?>
                        <li class="<?=$activeClass?>"><a href="car_waybills.php?id_car=<?=$car_id?>&page=<?=$i?>"><?=$i?></a></li>
                    <?
                }
            ?>
