...
class ReportClass
{
    ...
    var $report_id;
    var $users_buffer;
    var $rep_with_car_id = array(15, 35, 36, 37, 38, 40, 44);           //Отчеты с идентификатором автомобиля
    var $rep_with_fuel_type_id = array(35, 36);                     //Отчеты с идентификатором типа горючего
    var $rep_witout_dep_and_date_type = array(17,35,36,37,38, 40, 43, 44);  //идентификаторы отчетов без списка выбора департамента и типа даты
    ...
    public function GetReportNames()
    {
        ...
        $query = 'SELECT id, name, id_request FROM reports_info WHERE is_active = 1 AND id_request IN ('.$id_requests.') AND id NOT IN ('.$ids.') ORDER BY id_request, name';
